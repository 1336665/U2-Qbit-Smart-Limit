...
try:
    ...
    for t in torrents:
        ...
        if ratio >= min_ratio and seeding_time >= min_time_secs:
            hours = seeding_time / 3600
            matched.append((name, ratio, hours))
    
    if matched:
        print(f"  ✓ 发现 {len(matched)} 个符合删除条件的种子:")
        print("")
        for i, (name, ratio, hours) in enumerate(matched[:10]):
            print(f"    {i+1}. {name[:45]}")
            print(f"       分享率: {ratio:.2f}  做种: {hours:.1f}h")
        if len(matched) > 10:
...
PYTEST
            ...
            q|Q)
                ...
                # 保护分类
                echo -e "  ${D}保护分类中的种子不会被删除，多个用逗号分隔${N}"
                read -rp "  保护分类 (回车跳过): " prot_cats
                
                # 保护标签
                echo -e "  ${D}带有保护标签的种子不会被删除，多个用逗号分隔${N}"
                read -rp "  保护标签 (回车跳过): " prot_tags
                ...
                local cat_array='[]'
                if [[ -n "$prot_cats" ]]; then
                    cat_array=$(echo "$prot_cats" | tr ',' '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | jq -R . | jq -s .)
                fi
                
                # 转换保护标签为JSON数组
                local tag_array='[]'
                if [[ -n "$prot_tags" ]]; then
                    tag_array=$(echo "$prot_tags" | tr ',' '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | jq -R . | jq -s .)
                fi
                ...
                if jq ".cleanup.min_ratio = $ratio | \
                        ...
                        .cleanup.protected_categories = $cat_array | \
                        .cleanup.protected_tags = $tag_array | \
