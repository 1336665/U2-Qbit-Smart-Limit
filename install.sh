...
get_remote_ver() {
    ...
    # 方法1: 直接连接
    ver=$(curl -sL --connect-timeout 10 --max-time 30 "${GITHUB_RAW}/main.py" 2>/dev/null | grep -oP 'VERSION = "\K[^"]+' | head -1)
    
    # 方法2: 如果配置了代理，尝试使用代理
    if [[ -z "$ver" && -n "$proxy_cfg" ]]; then
        ver=$(curl -sL --connect-timeout 10 --max-time 30 -x "$proxy_cfg" "${GITHUB_RAW}/main.py" 2>/dev/null | grep -oP 'VERSION = "\K[^"]+' | head -1)
    fi
...
try:
    ...
    min_ratio = float(clean.get('min_ratio', 1.0))
    min_time_hours = int(clean.get('min_seeding_time_hours', 24))
    min_time_secs = min_time_hours * 3600
    
    print(f"  删种状态: {'已启用' if enabled else '未启用'}")
    print(f"  条件: 分享率≥{min_ratio}, 做种≥{min_time_hours}小时({min_time_secs}秒)")
    print("")
    ...
    if tg_token and tg_chat:
        ...
        print(f"  TG通知: 未配置 (token={bool(tg_token)}, chat_id={bool(tg_chat)})")
    print("")
    ...
    for t in torrents:
        ...
        if ratio >= min_ratio and seeding_time >= min_time_secs and is_seeding:
    ...
    if matched:
        print(f"  ✓ 发现 {len(matched)} 个符合删除条件的种子:")
        print("")
        for i, m in enumerate(matched[:10]):
            print(f"    {i+1}. {m['name'][:45]}")
            print(f"       分享率: {m['ratio']:.2f}  做种: {m['hours']:.1f}h  状态: {m['state']}")
        if len(matched) > 10:
            print(f"    ... 还有 {len(matched) - 10} 个")
        print("")
        print("  ✓ 检查完成")
        print("")
        print("  如果自动删种已启用但未删除，请检查:")
        print("  1. 服务是否正在运行: systemctl status qbit-smart-limit")
        print("  2. 查看日志: journalctl -u qbit-smart-limit -f")
    else:
        print("  ✓ 没有符合删除条件的种子")
        print("")
        # 显示一些接近条件的种子
        ...
        for t in torrents:
            ...
            if is_seeding and (ratio >= min_ratio * 0.5 or seeding_time >= min_time_secs * 0.5):
                hours = seeding_time / 3600
                almost.append((name, ratio, hours, state))
        
        if almost:
            print("  接近条件的种子:")
            for name, ratio, hours, state in almost[:5]:
                print(f"    - {name[:40]}")
                print(f"      R={ratio:.2f} T={hours:.1f}h ({state})")
except Exception as e:
    import traceback
    print(f"  ✗ 测试失败: {e}")
    traceback.print_exc()
PYTEST
            ...
            q|Q)
                ...
                echo -e "  ${C}━━━━━━━━━━━━ 一键配置自动删种 ━━━━━━━━━━━━${N}"
                echo ""
                echo -e "  ${D}本向导将引导您完成自动删种的配置${N}"
                echo -e "  ${D}自动删种使用主配置中的 qBittorrent 连接${N}"
                echo -e "  ${Y}警告: 配置完成后将自动启用删种功能!${N}"
                echo ""
                
                # 最小分享率
                read -rp "  最小分享率 [1.0]: " ratio
                ratio=${ratio:-1.0}
                
                # 最小做种时间
                read -rp "  最小做种时间 (小时) [24]: " hours
                hours=${hours:-24}
                ...
                if jq ".cleanup.min_ratio = $ratio | \
                        .cleanup.min_seeding_time_hours = $hours | \
                        .cleanup.interval_seconds = $interval | \
            ...
            8)
                ...
                echo -e "  ${C}━━━━━━━━━━━━ 空间规则配置 ━━━━━━━━━━━━━${N}"
                echo ""
                echo -e "  ${D}根据剩余空间和上传速度自动删种${N}"
                echo ""
                
                local space_enabled
                space_enabled=$(get_config_bool "cleanup_space_rules_enabled" "false")
                
                if [[ "$space_enabled" == "true" ]]; then
                    echo -e "  当前状态: ${G}已启用${N}"
                else
                    echo -e "  当前状态: ${R}未启用${N}"
                fi
                ...
                # 显示当前规则
                local r1_gb r1_up r2_gb r2_up r3_gb r3_up
                ...
                echo -e "  ${Y}当前规则:${N}"
                echo -e "  1. 剩余<${r1_gb}G, 上传<${r1_up}KiB/s → 删除"
                echo -e "  2. 剩余<${r2_gb}G, 已完成, 上传<${r2_up}KiB/s → 删除"
                echo -e "  3. 剩余<${r3_gb}G, 上传<${r3_up}KiB/s → 删除 (紧急)"
                echo ""
                
                echo -e "  ${W}选项:${N}"
                echo -e "     ${G}1${N}. 开关空间规则"
                echo -e "     ${G}2${N}. 修改规则1 (空间<10G)"
                echo -e "     ${G}3${N}. 修改规则2 (空间<20G+已完成)"
                echo -e "     ${G}4${N}. 修改规则3 (空间<5G,紧急)"
                echo -e "     ${D}0${N}. 返回"
                echo ""
                ...
                case "$sub_choice" in
                    1)
                        if [[ "$space_enabled" == "true" ]]; then
                            if jq '.cleanup_space_rules_enabled = false' "$CONFIG_FILE" > "$tmp_cfg" 2>/dev/null; then
                                mv "$tmp_cfg" "$CONFIG_FILE"
                                chmod 600 "$CONFIG_FILE"
                                warn "空间规则已禁用"
                            else
                            ...
                            if jq '.cleanup_space_rules_enabled = true' "$CONFIG_FILE" > "$tmp_cfg" 2>/dev/null; then
                                mv "$tmp_cfg" "$CONFIG_FILE"
                                chmod 600 "$CONFIG_FILE"
                                ok "空间规则已启用"
                            else
                    ...
                    2)
                        ...
                        new_gb=${new_gb:-$r1_gb}
                        new_up=${new_up:-$r1_up}
                        if jq ".cleanup_space_rule1_gb = $new_gb | .cleanup_space_rule1_upload_kib = $new_up" "$CONFIG_FILE" > "$tmp_cfg" 2>/dev/null; then
                            mv "$tmp_cfg" "$CONFIG_FILE"
                            chmod 600 "$CONFIG_FILE"
                            ok "规则1已更新: 空间<${new_gb}G, 上传<${new_up}KiB/s"
                        else
                            rm -f "$tmp_cfg"
                            err "更新失败"
                        fi
                    ...
                    3)
                        ...
                        new_gb=${new_gb:-$r2_gb}
                        new_up=${new_up:-$r2_up}
                        if jq ".cleanup_space_rule2_gb = $new_gb | .cleanup_space_rule2_upload_kib = $new_up" "$CONFIG_FILE" > "$tmp_cfg" 2>/dev/null; then
                            mv "$tmp_cfg" "$CONFIG_FILE"
                            chmod 600 "$CONFIG_FILE"
                            ok "规则2已更新: 空间<${new_gb}G, 已完成, 上传<${new_up}KiB/s"
                        else
                            rm -f "$tmp_cfg"
                            err "更新失败"
                        fi
                    ...
                    4)
                        ...
                        new_gb=${new_gb:-$r3_gb}
                        new_up=${new_up:-$r3_up}
                        if jq ".cleanup_space_rule3_gb = $new_gb | .cleanup_space_rule3_upload_kib = $new_up" "$CONFIG_FILE" > "$tmp_cfg" 2>/dev/null; then
                            mv "$tmp_cfg" "$CONFIG_FILE"
                            chmod 600 "$CONFIG_FILE"
                            ok "规则3已更新: 空间<${new_gb}G, 上传<${new_up}KiB/s"
                        else
                            rm -f "$tmp_cfg"
                            err "更新失败"
                        fi
