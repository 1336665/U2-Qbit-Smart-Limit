    ...
    def _init_modules(self):
        ...
        try:
            ...
            if self.config.subscription_enabled:
                ...
                self.logger.info("📥 订阅模块已初始化并启动")
            else:
                self.logger.info("📥 订阅模块已初始化 (未启用自动启动)")
        except Exception as e:
            self.logger.error(f"订阅模块初始化失败: {e}")
            self.subscription_module = None
        
        # 初始化删种模块（始终创建，根据配置决定是否自动启动）
        try:
            ...
            if self.config.cleanup_enabled:
                ...
                self.logger.info("🗑️ 删种模块已初始化并启动")
            else:
                self.logger.info("🗑️ 删种模块已初始化 (未启用自动启动)")
        except Exception as e:
            self.logger.error(f"删种模块初始化失败: {e}")
            self.cleanup_module = None
