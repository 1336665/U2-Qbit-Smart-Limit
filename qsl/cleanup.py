...
class CleanupModule:
    ...
    def _auto_cleanup(self):
        ...
        logger.debug(f"删种检查: ratio≥{min_ratio}, time≥{min_seeding_time}s ({min_seeding_time/3600:.1f}h)")
            ...
            for t in torrents:
                try:
                    ...
                    if should_delete_space:
                        logger.info(f"🗑️ 空间规则触发: {name[:30]} - {space_reason}")
                    ...
                    if ratio >= min_ratio and seeding_time >= min_seeding_time:
                        ...
                        logger.info(f"🗑️ 删种条件满足: {name[:30]} - {reason}")
