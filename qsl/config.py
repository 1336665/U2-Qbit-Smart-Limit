    ...
    def load(path: str, db: 'Database' = None) -> Tuple[Optional['Config'], Optional[str]]:
        try:
            ...
            cfg = Config(
                ...
                # 订阅模块
                subscription_enabled=bool(d.get('subscription_enabled', False)),
                subscription_interval=int(d.get('subscription_interval', 60) or 60),
                subscription_rss_url=str(d.get('subscription_rss_url', '')).strip(),
                subscription_download_path=str(d.get('subscription_download_path', '')).strip(),
                subscription_category=str(d.get('subscription_category', '')).strip(),
                subscription_paused=bool(d.get('subscription_paused', False)),
                subscription_first_last_piece=bool(d.get('subscription_first_last_piece', False)),
                # 删种模块
                cleanup_enabled=bool(d.get('cleanup_enabled', False)),
                cleanup_interval=int(d.get('cleanup_interval', 300) or 300),
                cleanup_min_ratio=float(d.get('cleanup_min_ratio', 1.0) or 1.0),
                cleanup_min_seeding_time=int(d.get('cleanup_min_seeding_time', 86400) or 86400),
                cleanup_delete_files=bool(d.get('cleanup_delete_files', False)),
                cleanup_tracker_keyword=str(d.get('cleanup_tracker_keyword', '')).strip(),
                cleanup_reannounce_before_delete=bool(d.get('cleanup_reannounce_before_delete', True)),
                cleanup_reannounce_wait=int(d.get('cleanup_reannounce_wait', 5) or 5),
                # 基于剩余空间的删种规则
