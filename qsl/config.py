...
class Config:
    ...
    # 基于剩余空间的删种规则（默认启用，无需开关）
    # 规则1: 剩余空间小于10G，上传小于1024KiB/s
    cleanup_space_rule1_gb: int = 10
    cleanup_space_rule1_upload_kib: int = 1024
    # 下载规则1: 剩余空间小于10G，下载小于1024KiB/s
    cleanup_space_rule1_download_kib: int = 1024
    # 规则2: 剩余空间小于20G，下载完成，上传小于512KiB/s
    cleanup_space_rule2_gb: int = 20
    cleanup_space_rule2_upload_kib: int = 512
    # 下载规则2: 剩余空间小于20G，下载小于512KiB/s
    cleanup_space_rule2_download_kib: int = 512
    # 规则3: 剩余空间小于5G，上传小于5120KiB/s (紧急)
    cleanup_space_rule3_gb: int = 5
    cleanup_space_rule3_upload_kib: int = 5120
    # 下载规则3: 剩余空间小于5G，下载小于5120KiB/s (紧急)
    cleanup_space_rule3_download_kib: int = 5120
    ...
    def load(path: str, db: 'Database' = None) -> Tuple[Optional['Config'], Optional[str]]:
        try:
            ...
            cfg = Config(
                ...
                cleanup_space_rule1_download_kib=int(d.get('cleanup_space_rule1_download_kib', 1024) or 1024),
                ...
                cleanup_space_rule2_download_kib=int(d.get('cleanup_space_rule2_download_kib', 512) or 512),
                ...
                cleanup_space_rule3_download_kib=int(d.get('cleanup_space_rule3_download_kib', 5120) or 5120),
                _mtime=mtime
            )
