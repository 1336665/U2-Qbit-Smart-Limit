...
class SubscriptionModule:
    ...
    def _process_rss(self):
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(f"RSS获取失败: HTTP {resp.status_code}")
                return
    ...
    def _add_torrent(self, url: str, name: str, source: str, 
        ...
        try:
            ...
            # 记录到数据库
            url_hash = hashlib.md5(url.encode()).hexdigest()
            self.db.add_subscription_history(url_hash, name, source)
            
            # TG通知
            if self.notifier:
                self.notifier.subscription_notify(name, size, source)
            
            logger.info(f"📥 添加种子: {name[:40]} ({source})")
            return True
