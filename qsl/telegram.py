...
└ 总上传: <code>{fmt_size(stats.uploaded)}</code>""")
    ...
    def _cmd_subscription(self, args: str):
        ...
        if sub_args == "status":
            if hasattr(self.controller, 'subscription_module') and self.controller.subscription_module:
                sub = self.controller.subscription_module
...
已添加种子: {count}个""")
                ...
                self.send_immediate("❌ 订阅模块未启用")
        elif sub_args == "start":
            if hasattr(self.controller, 'subscription_module') and self.controller.subscription_module:
                self.controller.subscription_module.start()
                self.send_immediate("✅ 订阅模块已启动")
            else:
                self.send_immediate("❌ 订阅模块未初始化")
        elif sub_args == "stop":
            if hasattr(self.controller, 'subscription_module') and self.controller.subscription_module:
                self.controller.subscription_module.stop()
...
/sub stop - 停止订阅""")
    ...
    def _cmd_cleanup(self, args: str):
        ...
        if cleanup_args == "status":
            if hasattr(self.controller, 'cleanup_module') and self.controller.cleanup_module:
                cleanup = self.controller.cleanup_module
