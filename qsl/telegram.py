    ...
    def _cmd_cleanup(self, args: str):
        ...
        if cleanup_args == "status":
            if cleanup:
                ...
                st = cleanup.get_status() or {}
                interval = st.get('interval', self.controller.config.cleanup_interval)
                delete_files = "✅" if st.get('delete_files', self.controller.config.cleanup_delete_files) else "❌"
                free_gb = st.get('free_space_gb', None)
                rules = st.get('space_rules', {}) or {}

                def _fmt_free():
                    if isinstance(free_gb, (int, float)) and free_gb != float('inf'):
                        return f"{free_gb:.1f}G"
                    if free_gb == float('inf'):
                        return "未知"
                    return "未知"

                r1_gb = rules.get('rule1_gb', self.controller.config.cleanup_space_rule1_gb)
                r1_up = rules.get('rule1_upload_kib', self.controller.config.cleanup_space_rule1_upload_kib)
                r2_gb = rules.get('rule2_gb', self.controller.config.cleanup_space_rule2_gb)
                r2_up = rules.get('rule2_upload_kib', self.controller.config.cleanup_space_rule2_upload_kib)
                r3_gb = rules.get('rule3_gb', self.controller.config.cleanup_space_rule3_gb)
                r3_up = rules.get('rule3_upload_kib', self.controller.config.cleanup_space_rule3_upload_kib)
...
<b>空间规则</b>（默认启用，仅做种/上传状态生效，优先级 3&gt;1&gt;2）
1) 剩余&lt;{r1_gb}G, 上传&lt;{r1_up}KiB/s → 删除
2) 剩余&lt;{r2_gb}G, 已完成, 上传&lt;{r2_up}KiB/s → 删除
3) 剩余&lt;{r3_gb}G, 上传&lt;{r3_up}KiB/s → 删除（紧急）
